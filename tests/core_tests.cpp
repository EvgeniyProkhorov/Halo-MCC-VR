...
#include "halo3_scripted_input_logic.h"
...
int main()
{
...

    {
        ...
        // H3EK identifies 0.15 as the Cortana/Gravemind input-ownership
        // signal. Nearby float representations qualify, but other authored
        // gameplay scales and non-finite targets must remain untouched.
        const Halo3ScriptedInputPolicy cortanaInput =
            ClassifyHalo3ScriptedInputScale(0.15f, 0.5f);
        const Halo3ScriptedInputPolicy nearbyCortanaInput =
            ClassifyHalo3ScriptedInputScale(
                kHalo3ScriptedInputSuppressionScale +
                    kHalo3ScriptedInputScaleTolerance * 0.5f,
                2.0f);
        const Halo3ScriptedInputPolicy ordinarySlowdown =
            ClassifyHalo3ScriptedInputScale(0.25f, 1.0f);
        const Halo3ScriptedInputPolicy trainingSlowdown =
            ClassifyHalo3ScriptedInputScale(0.775f, 60.0f);
        const Halo3ScriptedInputPolicy outsideTolerance =
            ClassifyHalo3ScriptedInputScale(
                kHalo3ScriptedInputSuppressionScale +
                    kHalo3ScriptedInputScaleTolerance * 2.0f,
                0.5f);
        const Halo3ScriptedInputPolicy invalidTarget =
            ClassifyHalo3ScriptedInputScale(
                std::numeric_limits<float>::quiet_NaN(), 0.5f);
        const Halo3ScriptedInputPolicy invalidDuration =
            ClassifyHalo3ScriptedInputScale(
                0.15f, std::numeric_limits<float>::infinity());
        Check(cortanaInput.action ==
                  Halo3ScriptedInputAction::SuppressVrTurning &&
              nearbyCortanaInput.action ==
                  Halo3ScriptedInputAction::SuppressVrTurning &&
              ordinarySlowdown.action == Halo3ScriptedInputAction::Ignore &&
              trainingSlowdown.action == Halo3ScriptedInputAction::Ignore &&
              outsideTolerance.action == Halo3ScriptedInputAction::Ignore &&
              invalidTarget.action == Halo3ScriptedInputAction::Ignore &&
              invalidDuration.action == Halo3ScriptedInputAction::Ignore,
            "Halo 3 scripted input suppresses VR turning only for the finite H3EK-authored 0.15 scale");

        // The setter's 1.0 footer marks restore-pending, but does not itself
        // authorize release. Native record completion below is authoritative.
        const Halo3ScriptedInputPolicy halfSecondRestore =
            ClassifyHalo3ScriptedInputScale(1.0f, 0.5f);
        const Halo3ScriptedInputPolicy oneSecondRestore =
            ClassifyHalo3ScriptedInputScale(
                kHalo3ScriptedInputRestoreScale -
                    kHalo3ScriptedInputScaleTolerance * 0.5f,
                1.0f);
        const Halo3ScriptedInputPolicy negativeRestore =
            ClassifyHalo3ScriptedInputScale(1.0f, -1.0f);
        const Halo3ScriptedInputPolicy invalidDurationRestore =
            ClassifyHalo3ScriptedInputScale(
                1.0f, std::numeric_limits<float>::infinity());
        Check(halfSecondRestore.action ==
                  Halo3ScriptedInputAction::RestoreVrTurningWhenNativeComplete &&
              oneSecondRestore.action ==
                  Halo3ScriptedInputAction::RestoreVrTurningWhenNativeComplete &&
              negativeRestore.action ==
                  Halo3ScriptedInputAction::Ignore &&
              invalidDurationRestore.action ==
                  Halo3ScriptedInputAction::Ignore,
            "Halo 3 scripted-input footer accepts only finite nonnegative native durations");

        Halo3ScriptedInputRecordState scriptedRecords[
            kHalo3ScriptedInputUserCount]{};
        scriptedRecords[0] = {
            true, 1, 1.0f, 0.15f, 0.5f};
        const bool enteringSuppressionRecord =
            Halo3ScriptedInputRecordShowsNativeSuppression(
                scriptedRecords[0]);
        const Halo3ScriptedInputObservation enteringSlowdown =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Suppressed,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 1, 0.15f, 0.15f, 0.0f};
        const bool heldSuppressionRecord =
            Halo3ScriptedInputRecordShowsNativeSuppression(
                scriptedRecords[0]);
        const Halo3ScriptedInputObservation heldSlowdown =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Suppressed,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 1, 0.15f, 1.0f, 0.5f};
        const Halo3ScriptedInputObservation pausedRestore =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Restoring,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 0, 1.0f, 1.0f, 0.0f};
        const Halo3ScriptedInputObservation completedRestore =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Restoring,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 0, 1.0f, 1.0f, 0.4f};
        const Halo3ScriptedInputObservation duplicateUnityRestore =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Restoring,
                scriptedRecords, 0x1);
        Check(enteringSlowdown ==
                  Halo3ScriptedInputObservation::NativeSuppressed &&
              enteringSuppressionRecord && heldSuppressionRecord &&
              heldSlowdown ==
                  Halo3ScriptedInputObservation::NativeSuppressed &&
              pausedRestore ==
              ...
              duplicateUnityRestore ==
                  Halo3ScriptedInputObservation::Restored,
            "Halo 3 scripted input stays owned through the 0.15 hold and releases only on native simulation-time restore completion");

        scriptedRecords[0] = {
            true, 1, 0.15f, 0.15f, 0.0f};
        scriptedRecords[1] = {
            true, 1, 0.15f, 1.0f, 0.5f};
        const Halo3ScriptedInputObservation mixedSetterWrite =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Suppressed,
                scriptedRecords, 0x3);
        scriptedRecords[0] = {
            false, 1, 0.15f, 1.0f, 0.5f};
        const Halo3ScriptedInputObservation unstableSnapshot =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Restoring,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 0, 0.25f, 0.25f, 0.0f};
        const Halo3ScriptedInputObservation unrelatedReplacement =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Suppressed,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 0, 0.15f, 0.15f, 0.0f};
        const bool inactiveSuppressionRecord =
            Halo3ScriptedInputRecordShowsNativeSuppression(
                scriptedRecords[0]);
        const Halo3ScriptedInputObservation inactiveSuppression =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Suppressed,
                scriptedRecords, 0x1);
        scriptedRecords[0] = {
            true, 0, std::numeric_limits<float>::quiet_NaN(),
            1.0f, 0.0f};
        const Halo3ScriptedInputObservation corruptRecord =
            ObserveHalo3ScriptedInputRecords(
                Halo3ScriptedInputPhase::Restoring,
                scriptedRecords, 0x1);
        Check(mixedSetterWrite == Halo3ScriptedInputObservation::Hold &&
              unstableSnapshot == Halo3ScriptedInputObservation::Hold &&
              unrelatedReplacement ==
                  Halo3ScriptedInputObservation::Invalid &&
              inactiveSuppression ==
                  Halo3ScriptedInputObservation::Invalid &&
              !inactiveSuppressionRecord &&
              corruptRecord == Halo3ScriptedInputObservation::Invalid,
            "Halo 3 scripted-input observation retries native transition races and fails unrelated or corrupt state open");
